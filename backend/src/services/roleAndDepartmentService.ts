...
// export async function updateRole(updateData: any) {
//   try {
...
//     if (!updateData.role_id) throw new Error("role_id is required");
...
//     return await client.query(query, [...values, updateData.role_id]);
//   } catch (error:any) {
//     if ((error as any).code === "23505") throw new Error("Role code must be unique");
//     throw new Error((error as Error).message);
//   }
  ...
  return withClient(async (client: any) => {
    ...
    `;
      ...
      `;
        ...
        rolesMap = roleResult.rows.reduce((acc: any, role: any) => {
      ...
      const enrichedDepartments = departments.map((dep: any) => {
